...
import { crypto_core_ed25519_add, crypto_scalarmult_ed25519_base_noclamp, } from "./sumo.facade.js";
...
export function fromSeed(seed) {
    ...
    // chain root code
    // SHA256(0x01||k)
    const c = createHash("sha256").update(Buffer.concat([new Uint8Array([0x01]), seed])).digest();
 ...
 */
export function trunc_256_minus_g_bits(array, g) {
    if (g < 0 || g > 256) {
 ...
 */
export async function deriveChildNodePrivate(extendedKey, index, g = 9) {
 ...
 */
export async function deriveChildNodePublic(extendedKey, index, g = 9) {
    if (index > 0x80000000)
        throw Error('can not derive public key with harden');
    ...
    const p = crypto_scalarmult_ed25519_base_noclamp(left);
...
//# sourceMappingURL=bip32-ed25519.js.map